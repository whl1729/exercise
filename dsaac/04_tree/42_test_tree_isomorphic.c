...
int tree_is_isomorphic(Tree *first, Tree *second);
...
int main()
{
    ...
    while (go_on)
    {
        ...
        if (tree_is_isomorphic(first, second))
        {
...
int tree_is_isomorphic(Tree *first, Tree *second)
{
    ...
    {
        return ((tree_is_isomorphic(first->left, second->left) &&
                 tree_is_isomorphic(first->right, second->right)) ||
                (tree_is_isomorphic(first->left, second->right) &&
                 tree_is_isomorphic(first->right, second->left)));
    }
    else
    {
        return FALSE;
