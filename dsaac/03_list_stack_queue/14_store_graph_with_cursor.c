...
int init_graph(int **graph, int vertex_num);
int read_graph(int *graph, int vertex_num, int edge_num);
int print_graph(int *graph, int vertex_num);
int clear_graph(int *graph, int vertex_num);
...
int init_graph(int **in_graph, int vertex_num)
{
    int result = SUCCESS;
    int vertex_id = 0;
    int *graph = NULL;
    ...
    memset(graph, 0, sizeof(int) * vertex_num);

    cursor_init();

    for (vertex_id = 0; vertex_id < vertex_num; vertex_id++)
    {
        graph[vertex_id] = list_init();
        if (0 == graph[vertex_id])
        {
