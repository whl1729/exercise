...
{
public:
    TextQuery(): file(new StrVec), word_count(make_shared<Msi>()) {}
    TextQuery(const string &name): file(new StrVec), word_count(new Msi) 
    {
        ifstream input(name);
        read(input);
        input.close();
...
public:
    QueryResult() = default;
    QueryResult(const string &w, int c, shared_ptr<StrVec> t, shared_ptr<Iset> l): word(w), count(c), file(t), line(l) {}
